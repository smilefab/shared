...
def experiment(args, idx):
    ...
    for n_epoch in range(1, max_steps // evaluation_frequency + 1):
        ...
        if args.save:
            np.save(folder_name + 'best_weights-exp-%d-%d.npy' % (idx, n_epoch),
    ...
    return scores, agent._critic_approximator.model._loss.get_losses(), np.array(core._tasks_list)
...
if __name__ == '__main__':
    ...
    arg_game.add_argument("--games",
                          type=list,
                          nargs='+',
                          default=['InvertedPendulumBulletEnvv0'],
    ...
    arg_alg.add_argument("--target-update-frequency", type=int, default=1,
    ...
    arg_alg.add_argument("--initial-prism-rate", type=int, default=1,
                         help="The initial epsilon for PRISM sampling.")
    arg_alg.add_argument("--final-prism-rate", type=int, default=0,
                         help="The final epsilon for PRISM sampling.")
    arg_alg.add_argument("--final-prism-step", type=int, default=7500000,
                         help="Determines when the final epsilon is reached.")
    ...
    if not len(sys.argv) > 1:
        args_str =  '--features sigmoid ' \
                    '--n-exp 1 ' \
                    '--games InvertedPendulum-v2 ' \
                           'InvertedDoublePendulum-v2 ' \
                    '--gamma .99 .99 .99 ' \
                    '--horizon 1000 1000 1000 ' \
                    '--sampling uniform' 
    ...
    folder_name = './logs/ddpg/gym_' + datetime.datetime.now().strftime(
